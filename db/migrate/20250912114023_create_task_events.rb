class CreateTaskEvents < ActiveRecord::Migration[8.0]
  def change
    create_table :task_events, comment: "タスクイベント" do |t|
      t.references :task,  null: false, foreign_key: true, comment: '対象タスクID'
      ...
      t.references :assignee_from, null: true, foreign_key: { to_table: :workers }, comment: '変更前の担当者ID'
      t.references :assignee_to, null: true, foreign_key: { to_table: :workers }, comment: '変更後の担当者ID'
      t.date :due_on_from, null: true, comment: '変更前の期限'
      t.date :due_on_to, null: true, comment: '変更後の期限'
      t.references :task_comment, null: true, foreign_key: true, comment: '関連コメントID'
      t.references :work, null: true, foreign_key: true, comment: '関連作業ID'
