...
class PersonalInformationsTest < ApplicationSystemTestCase
  setup do
    @user = users(:users1)
    @work = works(:work_for_show_user1)
  end
  ...
  test "日報表示(標準)" do
    visit personal_information_work_path(personal_information_token: @user.token, id: @work.id)

    assert_selector 'nav'
  end

  test "日報表示(簡易)" do
    visit personal_information_work_path(personal_information_token: @user.token, id: @work.id, simple: true)
