...
class MachineReserveFundTest < ActiveSupport::TestCase
  ...
  test "必須項目が揃っていれば登録できる" do
    reserve_fund = MachineReserveFund.new(
      organization: @organization, machine: @machine,
      started_on: Date.new(2025, 4, 1), years: 7, total_amount: 3_500_000
    )
    assert reserve_fund.valid?
  end

  test "started_on/total_amountが無いと登録できない" do
  ...
  test "登録済み明細の合計額より総額を小さくできない" do
    reserve_fund = build_reserve_fund(total_amount: 1_000_000).tap(&:save!)
    create_detail(reserve_fund, amount: 400_000, remaining_amount: 600_000)

    reserve_fund.total_amount = 300_000
    assert_not reserve_fund.valid?
    assert_includes reserve_fund.errors.attribute_names, :total_amount

    reserve_fund.total_amount = 400_000
    assert reserve_fund.valid?
  end

  test "registered_amount/remaining_amountは明細の合計から算出される" do
    reserve_fund = build_reserve_fund(total_amount: 1_000_000).tap(&:save!)
    assert_equal 0, reserve_fund.registered_amount
    assert_equal 1_000_000, reserve_fund.remaining_amount
    assert_not reserve_fund.details?

    create_detail(reserve_fund, amount: 150_000, remaining_amount: 850_000)

    assert_equal 150_000, reserve_fund.registered_amount
    assert_equal 850_000, reserve_fund.remaining_amount
  ...
  def build_reserve_fund(total_amount:, started_on: Date.new(2025, 4, 1), years: 7)
    MachineReserveFund.new(
      organization: @organization, machine: @machine, started_on: started_on, years: years, total_amount: total_amount
    )
  end

  def create_detail(reserve_fund, amount:, remaining_amount:)
    MachineReserveFundDetail.create!(
      organization: @organization, machine_reserve_fund: reserve_fund,
      term: 2025, months: 12, amount: amount, remaining_amount: remaining_amount
    )
  end
end
