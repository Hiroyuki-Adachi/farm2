...
class IpListsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "番号認証(LINE認証)(return_to指定でタブレットへ遷移)" do
    ip = IpList.white_ip!(@ip_address, @user)
    patch ip_list_path(id: ip.id), params: { token: ip.token, return_to: tablets_menu_index_path }, headers: { "REMOTE_ADDR" => @ip_address }
    assert_redirected_to tablets_menu_index_path
