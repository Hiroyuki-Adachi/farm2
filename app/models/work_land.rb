...
class WorkLand < ApplicationRecord
  ...
  scope :for_fix, lambda { |term, fixed_at, contract_id, organization = nil|
    base = joins(:work)
      .joins(:land)
  ...
  scope :for_cards, lambda { |land_id, worked_at|
  ...
  def largest?
    ...
    land.id == work.lands.select { |land| land.area == max_area }.map(&:id).min
  end
  ...
  def same_areas
    WorkLand.sum_areas(work_id, work_type_id, work.chemical_group_flag ? chemical_group_no : 0)
  end
  ...
  def total_sunshine(organization)
    ...
    DailyWeather.sum_sunshine(rice_plant_work.worked_at, work.worked_at)
  end
