...
#  drying_type(乾燥種別)  :integer          default(0), not null
...
class Drying < ApplicationRecord
  ...
  enum :drying_type, {none: 0, country: 1, self: 2, another: 3, adjust: 4}, prefix: true
  ...
  def harvest_weight(system)
    return rice_weight || 0 if self.drying_type_country?
    return adjustment&.rice_weight(system) || 0
  end

  def waste_weight
    return 0 if self.drying_type_country?
  ...
  def adjust_only?(home_id)
    return self.drying_type_another? && adjustment&.home_id == home_id
  end

  def delete_child
    if self.drying_type_country?
      adjustment.destroy
    else
  ...
  def price(system, home_id)
    ...
    return self.drying_type_self? ? system.dry_adjust_price : system.dry_price
  end
  ...
  def drying_type_name
    I18n.t("activerecord.attributes.dryings.drying_types.#{drying_type}")
  end
end
