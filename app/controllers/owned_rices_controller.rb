class OwnedRicesController < ApplicationController
  ...
  def edit
    ...
    @p_relative_prices = OwnedRicePrice.relative.usual(previous_term).to_a
  ...
  def update
    params.require(:owned_rices).each do |_k, v|
      OwnedRice.regist(v[:id], v.permit(:home_id, :owned_rice_price_id, :owned_count, :relative_count))
    end
