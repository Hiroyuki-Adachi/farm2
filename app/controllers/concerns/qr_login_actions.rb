module QrLoginActions
  ...
  def consume
    ...
    reset_session
    log_in(user, target: login_target)
    render json: { ok: true, action: "redirect", url: redirect_path }
  end

  private

  # 継承先で必ず固定値を返すこと。redirect_to 等のリクエスト値からは絶対に決定しない
  # （タブレット用の無制限エンドポイントからPCターゲットを騙し取れてしまうため）。
  def login_target
    :PC
  end

  # 継承先でリダイレクト先として許可するパスのプレフィックスを返す。nilなら常にdefault_redirect_pathを使う
  ...
  def redirect_path
    safe_redirect_to_path(params[:redirect_to]) || normalize_internal_path(default_redirect_path)
  end

  def safe_redirect_to_path(path)
    return if path.blank?

    prefix = allowed_redirect_prefix
